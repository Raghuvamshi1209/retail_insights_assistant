...
from retail_ai.data_engine.sql_builder import SAFE_METRICS


def validate_sql_is_select(sql: str):
...
def validate_plan(plan: Dict[str, Any], schema_cols: List[str]):
    warnings: List[str] = []
    ...
    plan["metrics"] = cleaned_metrics or ["shipped_amount"]
